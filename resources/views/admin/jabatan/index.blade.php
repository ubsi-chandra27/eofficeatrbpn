...
<div class="position-card fade-up">
    <div class="position-toolbar">
        ...
        <form method="GET" action="{{ route('admin.jabatan.index') }}" class="position-search">
            <div class="search-input"><i class="bi bi-search"></i><input name="keyword" value="{{ request('keyword') }}" placeholder="Cari kode, nama, atau deskripsi..."></div>
            <select name="penggunaan" class="form-select" data-native-select><option value="">Semua Penggunaan</option><option value="terpakai" @selected(request('penggunaan')==='terpakai')>Digunakan Pegawai</option><option value="kosong" @selected(request('penggunaan')==='kosong')>Belum Digunakan</option></select>
            <button class="btn btn-primary" title="Terapkan filter"><i class="bi bi-funnel"></i></button>
            @if(request()->hasAny(['keyword','penggunaan']))<a href="{{ route('admin.jabatan.index') }}" class="btn btn-light border" title="Hapus filter"><i class="bi bi-x-lg"></i></a>@endif
        </form>
    </div>
...
.position-search{position:static;display:grid;grid-template-columns:minmax(220px,1fr) 180px auto auto;gap:8px;width:min(650px,100%)}.search-input{position:relative}.search-input>i{position:absolute;left:14px;top:50%;transform:translateY(-50%);color:#94a3b8}.position-search .search-input input{padding-left:42px}.position-search select{height:46px;border-radius:11px;border-color:#dbe2ea}.position-search>a{position:static;transform:none;color:inherit}@media(max-width:900px){.position-search{grid-template-columns:1fr 1fr}}@media(max-width:600px){.position-search{grid-template-columns:1fr}}
</style>@endpush
