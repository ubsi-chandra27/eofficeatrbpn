...
<div class="submission-page">
    ...
    <div class="submission-card">
        <form method="GET" class="submission-filter">
            <div class="search-field"><i class="bi bi-search"></i><input name="keyword" value="{{ request('keyword') }}" placeholder="Cari nomor, pokok, kontak, atau instansi..."></div>
        ...
        <div class="table-responsive"><table class="table align-middle mb-0 submission-table"><thead><tr><th width="55">No</th><th>Nomor Pengajuan</th><th>Kategori &amp; Pokok Pengajuan</th><th>Kontak &amp; Instansi</th><th>Waktu Pengajuan</th><th>Lampiran</th><th>Status &amp; Tahap</th><th class="text-end">Aksi</th></tr></thead><tbody>
        @forelse($surats as $surat)
            ...
            <tr class="{{ in_array($surat->status,['dikembalikan','ditolak']) ? 'needs-revision' : '' }}">
                ...
                <td><strong class="submission-number">{{ $surat->nomor_surat }}</strong></td>
                ...
                <td><span>{{ optional($surat->tanggal_surat)->format('d M Y') }}</span><small>Diperbarui {{ $surat->updated_at?->diffForHumans() }}</small></td>
                <td>@if($fileExtension)<a class="file-chip" href="{{ route('umum.surat.download',$surat->id) }}" title="Unduh lampiran"><i class="bi bi-file-earmark-arrow-down"></i>{{ $fileExtension }}</a>@else<span class="text-muted">Tidak ada</span>@endif</td>
                <td><span class="badge rounded-pill bg-{{ $surat->status_badge }}">{{ $surat->status_label }}</span><small class="process-stage"><i class="bi bi-diagram-3"></i>{{ $processStage }}</small>@if($surat->catatan_admin)<small class="admin-note" title="{{ $surat->catatan_admin }}"><i class="bi bi-chat-left-text"></i>{{ $surat->catatan_admin }}</small>@endif</td>
                <td><div class="submission-actions"><a href="{{ route('umum.surat.show',$surat->id) }}" class="view" title="Lihat detail"><i class="bi bi-eye"></i></a>@if($fileExtension)<a href="{{ route('umum.surat.download',$surat->id) }}" class="download" title="Unduh lampiran"><i class="bi bi-download"></i></a>@endif @if(in_array($surat->status,['menunggu','dikembalikan','ditolak']))<a href="{{ route('umum.surat.edit',$surat->id) }}" class="edit" title="Perbaiki pengajuan"><i class="bi bi-pencil-square"></i></a>@endif</div></td>
            </tr>
        @empty
            <tr><td colspan="8"><div class="empty-submission"><i class="bi bi-inbox"></i><h5>{{ request()->hasAny(['keyword','kategori','status']) ? 'Pengajuan tidak ditemukan' : 'Belum ada pengajuan' }}</h5><p>{{ request()->hasAny(['keyword','kategori','status']) ? 'Ubah atau hapus filter untuk melihat data lainnya.' : 'Pengajuan yang Anda kirim akan tampil dan dapat dipantau di halaman ini.' }}</p>@if(request()->hasAny(['keyword','kategori','status']))<a href="{{ route('umum.surat.index') }}" class="btn btn-outline-primary btn-sm">Hapus Filter</a>@endif</div></td></tr>
        @endforelse
        </tbody></table></div>
...
.submission-table{width:100%;min-width:1160px;table-layout:fixed}.submission-table th:nth-child(1){width:5%}.submission-table th:nth-child(2){width:14%}.submission-table th:nth-child(3){width:21%}.submission-table th:nth-child(4){width:15%}.submission-table th:nth-child(5){width:13%}.submission-table th:nth-child(6){width:8%}.submission-table th:nth-child(7){width:16%}.submission-table th:nth-child(8){width:8%}.submission-table td{overflow-wrap:anywhere}.file-chip{display:inline-flex!important;align-items:center;gap:5px;padding:6px 9px;border-radius:8px;background:#eef6ff;color:#1769aa;text-decoration:none;font-weight:700}.process-stage{max-width:210px;color:#526174!important;margin-top:6px!important}.process-stage i{margin-right:5px}.submission-actions .download{background:#ecfdf5;color:#047857}
</style>@endpush
