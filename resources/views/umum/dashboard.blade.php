...
<div class="public-dashboard">
    ...
    @if($statistik['total'] === 0)
        <div class="dashboard-card latest-submission-card mb-3">
            ...
            <div class="table-responsive">
                <table class="table latest-submission-table mb-0">
                    <thead><tr><th>Nomor Pengajuan</th><th>Kategori &amp; Pokok</th><th>Kontak &amp; Instansi</th><th>Tanggal</th><th>Lampiran</th><th>Status &amp; Tahap</th><th class="text-end">Aksi</th></tr></thead>
                    <tbody><tr><td colspan="7"><div class="empty-box"><i class="bi bi-inbox"></i><strong>Belum ada pengajuan</strong><span>Buat pengajuan pertama agar prosesnya dapat dipantau dari dashboard.</span><a href="{{ route('umum.surat.create') }}" class="btn btn-primary btn-sm mt-3"><i class="bi bi-plus-circle me-1"></i>Buat Pengajuan</a></div></td></tr></tbody>
                </table>
            </div>
        </div>
    ...
    @else
        <div class="dashboard-card latest-submission-card">
            <div class="card-heading"><div><h5>Pengajuan Terbaru</h5><small>Lima pengajuan terakhir milik Anda</small></div><a href="{{ route('umum.surat.index') }}">Lihat semua</a></div>
            <div class="table-responsive">
                <table class="table latest-submission-table mb-0">
                    <thead><tr><th>Nomor Pengajuan</th><th>Kategori &amp; Pokok</th><th>Kontak &amp; Instansi</th><th>Tanggal</th><th>Lampiran</th><th>Status &amp; Tahap</th><th class="text-end">Aksi</th></tr></thead>
                    <tbody>
                    @foreach($suratTerbaru as $surat)
                        @php
                            $processStage = match ($surat->status) {
                                ...
                                'dikembalikan', 'ditolak' => 'Harus diperbaiki',
                                'selesai', 'terkirim', 'diarsipkan' => 'Proses telah selesai',
                                default => $surat->status_label,
                            };
                            $fileExtension = $surat->file_path ? strtoupper(pathinfo($surat->file_path, PATHINFO_EXTENSION)) : null;
                        @endphp
                        <tr>
                            <td><strong>{{ $surat->nomor_surat }}</strong></td>
                            <td><small class="category-text">{{ $surat->kategori_pengajuan ?? 'Pengajuan Umum' }}</small><span>{{ Str::limit($surat->perihal, 42) }}</span></td>
                            <td><span>{{ $surat->nomor_kontak ?: '-' }}</span><small>{{ $surat->asal_instansi ?: 'Perorangan' }}</small></td>
                            <td><span>{{ $surat->tanggal_surat?->translatedFormat('d M Y') ?? '-' }}</span><small>Diperbarui {{ $surat->updated_at?->diffForHumans() }}</small></td>
                            <td>@if($fileExtension)<span class="file-format"><i class="bi bi-paperclip"></i>{{ $fileExtension }}</span>@else<span class="text-muted">Tidak ada</span>@endif</td>
                            <td><span class="badge bg-{{ $surat->status_badge }}">{{ $surat->status_label }}</span><small>{{ $processStage }}</small>@if($surat->catatan_admin)<small class="admin-note">{{ Str::limit($surat->catatan_admin, 45) }}</small>@endif</td>
                            <td class="text-end"><a href="{{ route('umum.surat.show',$surat->id) }}" class="btn btn-sm btn-outline-primary" title="Lihat pengajuan"><i class="bi bi-eye"></i></a></td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
        ...
        <aside class="service-panel">
            <div class="service-row">
                ...
                <div>
                    ...
                    <b>{{ str_replace('–', '-', $informasiLayanan['jam_layanan']) }}</b>
                </div>
            </div>
            <div class="service-row">
                <i class="bi bi-paperclip"></i>
                <div>
                    <small>Ketentuan Lampiran</small>
                    <b>PDF, DOC, DOCX, JPG, PNG</b>
                    <span>Maksimal {{ $informasiLayanan['maksimal_lampiran'] }} MB per pengajuan.</span>
                </div>
            </div>
            @if($informasiLayanan['email_bantuan'] || $informasiLayanan['telepon_bantuan'])
                <div class="service-row">
                    <i class="bi bi-headset"></i>
                    <div>
                        <small>Kontak Bantuan</small>
                        @if($informasiLayanan['email_bantuan'])
                            <a href="mailto:{{ $informasiLayanan['email_bantuan'] }}">{{ $informasiLayanan['email_bantuan'] }}</a>
                        @endif
                        @if($informasiLayanan['telepon_bantuan'])
                            <a href="tel:{{ preg_replace('/[^0-9+]/', '', $informasiLayanan['telepon_bantuan']) }}">{{ $informasiLayanan['telepon_bantuan'] }}</a>
                        @endif
                    </div>
                </div>
            @else
                <div class="service-row">
                    ...
                    <div>
                        <small>Kontak Bantuan</small>
                        <span class="text-muted">Kontak layanan belum diatur oleh admin.</span>
                    </div>
                </div>
            @endif
            <div class="data-protection">
                ...
                <div>
                    <b>Perlindungan Data</b>
                    <span>Pengajuan hanya terlihat oleh pemilik akun dan petugas yang berwenang.</span>
                </div>
            </div>
        </aside>
    </section>
    ...
    <section class="faq-section"><div class="info-heading"><span class="section-label">BANTUAN</span><h4>Pertanyaan yang sering diajukan</h4></div><div class="accordion accordion-flush" id="publicFaq">
        ...
        <div class="accordion-item"><h2 class="accordion-header"><button class="accordion-button {{ $index ? 'collapsed' : '' }}" type="button" data-bs-toggle="collapse" data-bs-target="#faq{{ $index }}" aria-expanded="{{ $index === 0 ? 'true' : 'false' }}" aria-controls="faq{{ $index }}">{{ $question }}</button></h2><div id="faq{{ $index }}" class="accordion-collapse collapse {{ $index === 0 ? 'show' : '' }}" data-bs-parent="#publicFaq"><div class="accordion-body">{{ $answer }}</div></div></div>
        @endforeach
    </div></section>
</div>
@endsection
...
.latest-submission-card{height:auto}.latest-submission-table{min-width:1100px}.latest-submission-table th{padding:12px 10px;background:#f8fafc;color:#64748b;font-size:10px;letter-spacing:.5px;text-transform:uppercase;white-space:nowrap}.latest-submission-table td{padding:13px 10px;color:#475569;font-size:11px;vertical-align:middle;border-bottom-color:#edf1f5}.latest-submission-table td>strong,.latest-submission-table td>span,.latest-submission-table td>small{display:block}.latest-submission-table td>small{color:#8491a3;margin-top:2px}.latest-submission-table .category-text{color:#1769aa;font-weight:700}.latest-submission-table .admin-note{color:#b45309}.latest-submission-table .file-format{display:inline-flex;align-items:center;gap:4px;padding:5px 8px;border-radius:8px;background:#e0f2fe;color:#0369a1;font-weight:700}.latest-submission-table .btn{width:31px;height:31px;padding:0;display:inline-grid;place-items:center}
</style>@endpush
@push('styles')<style>
