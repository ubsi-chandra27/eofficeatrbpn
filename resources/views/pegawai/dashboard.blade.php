...
<div class="container-fluid employee-page employee-dashboard">
    ...
    <section class="dashboard-panel mb-4">
        ...
        <div class="table-responsive">
            <table class="table dashboard-table mb-0">
                <thead><tr><th>Nomor Surat</th><th>Perihal</th><th>Instruksi</th><th>Pengirim</th><th>Prioritas</th><th>Tanggal</th><th>Status</th><th class="text-end">Aksi</th></tr></thead>
                <tbody>
                    @forelse($disposisiTerbaru as $item)
                        @php
                            $surat = $item->disposisi?->surat;
                        @endphp
                        <tr class="{{ $item->status === 'Belum Dibaca' ? 'table-warning' : '' }}">
                            ...
                            <td>{{ $item->disposisi?->pengirim?->name ?? '-' }}</td>
                            ...
                            <td><strong>{{ $item->disposisi?->tanggal_disposisi?->translatedFormat('d M Y') ?? $item->created_at?->translatedFormat('d M Y') }}</strong><small class="d-block text-muted">{{ $item->updated_at?->format('H:i') }} WIB</small></td>
                    ...
                    @empty
                        <tr><td colspan="8"><div class="empty-state compact-empty"><i class="bi bi-inbox"></i><h5>Belum ada disposisi</h5><p>Tugas disposisi baru akan tampil di tabel ini.</p></div></td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <section class="dashboard-panel mb-4">
        ...
        <div class="table-responsive">
            <table class="table dashboard-table mb-0">
                <thead><tr><th>Nomor Surat</th><th>Perihal</th><th>Asal Surat</th><th>Tanggal</th><th>Lampiran</th><th>Status &amp; Tahap</th><th class="text-end">Aksi</th></tr></thead>
                <tbody>
                    @forelse($suratTerbaru as $surat)
                        @php
                            $verificationStage = match ($surat->status) {
                        ...
                        <tr>
                            ...
                            <td><span class="badge rounded-pill bg-{{ $surat->status_badge }}">{{ $surat->status_label }}</span><small class="d-block text-muted mt-1">{{ $verificationStage }}</small>@if($surat->catatan_admin)<small class="d-block text-danger mt-1">{{ Str::limit($surat->catatan_admin, 50) }}</small>@endif</td>
                    ...
                    @empty
                        <tr><td colspan="7"><div class="empty-state compact-empty"><i class="bi bi-envelope-arrow-down"></i><h5>Belum ada surat masuk</h5><p>Surat yang Anda catat akan tampil di tabel ini.</p></div></td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
...
.file-format{display:inline-flex;align-items:center;gap:5px;padding:5px 8px;border-radius:8px;background:#e0f2fe;color:#0369a1;font-weight:700}
</style>@endpush
